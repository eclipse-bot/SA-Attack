...
def objective(x, data, obj_id, predictor, loss_func, attack_opts):
    ...
    for n in range(N):
        ...
        ready_perturbation_tensor = hard_constraint(observe_trace_array, perturbation_tensor, attack_opts["bound"],
                                                    attack_opts["physical_bounds"])
        for k in range(attack_duration):
            # perturbation structure
            perturbation = {"obj_id": obj_id, "loss": loss_func,
                            "ready_value": {obj_id: ready_perturbation_tensor[k:k + obs_length, :]},
...
class PSOAttacker(BaseAttacker):
    def __init__(self, obs_length, pred_length, attack_duration, predictor, n_particles=10, iter_num=100, c1=0.5,
                 c2=0.3, w=1.0, bound=1, physical_bounds={}):
    ...
    def run(self, data, obj_id, **attack_opts):
        ...
        optimizer = ps.single.GlobalBestPSO(n_particles=self.n_particles, dimensions=self.perturb_length * 2,
                                            options=self.options, bounds=(
            -np.ones(self.perturb_length * 2) * self.bound, np.ones(self.perturb_length * 2) * self.bound),
                                            center=np.zeros(self.perturb_length * 2))
        best_loss, best_perturb = optimizer.optimize(objective, iters=self.iter_num, data=data, obj_id=str(obj_id),
                                                     predictor=self.predictor, loss_func=self.loss,
                                                     attack_opts=attack_opts)
        ...
        for k in range(self.attack_duration):
            # perturbation structure
            perturbation = {"obj_id": obj_id, "loss": self.loss,
                            "ready_value": {obj_id: ready_perturbation_tensor[k:k + self.obs_length, :]},
                            "attack_opts": attack_opts}
            # input data
