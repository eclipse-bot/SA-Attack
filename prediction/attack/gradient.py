    ...
    def run(self, data, obj_id, **attack_opts):
        ...
        for seed in range(self.seed_num):
            ...
            for _obj_id in perturbation["value"]:
                perturbation["value"][_obj_id] = Variable(torch.rand(self.obs_length + self.attack_duration - 1, 2).cuda() * 2 * self.bound - 1.0 * self.bound)

            #Optimize each disturbance 100 times
