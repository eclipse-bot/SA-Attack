...
def hard_constraint(observe_trace_array, perturbation_tensor, hard_bound, physical_bounds):
    ...
    while not check_pass:
        ...
        # check_pass = (np.sum(scalar_v > physical_bounds["scalar_v"]) == 0 and np.sum(linear_a > physical_bounds["linear_a"]) == 0 and np.sum(rotate_a > physical_bounds["rotate_a"]) == 0 and np.sum(linear_aa > physical_bounds["linear_aa"]) == 0 and np.sum(rotate_aa > physical_bounds["rotate_aa"]) == 0 and np.sum(deviation > 1) == 0)
        check_pass = (np.sum(deviation > 1) == 0)
