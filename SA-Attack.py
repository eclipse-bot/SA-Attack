...
def attack(model_name, dataset_name, overwrite=0, mode="single_frame", augment=False, smooth=0, blackbox=False):
    ...
    if not blackbox:
        attacker = GradientAttacker(api.obs_length, api.pred_length, attack_length, api, seed_num=10, iter_num=50, physical_bounds=physical_bounds, bound=1, learn_rate=0.1)
    else:
