...
class Trajectory:
    ...
    def getTargetPoint(self, pos):
        ...
        if screen_list:
            ...
            print("车辆偏移超出追踪距离")#TODO better process
            sys.exit()
        return target_point

def smooth_constraint(data, obj, perturbation, attack_length, look_distance, insert_num):
    #total_traj = data["objects"][obj]["observe_trace"]
    #heading = data["objects"][obj]["observe_feature"][:,4]
    if not isinstance(perturbation, np.ndarray):#判断类型是否为np.ndarray，并转换类型
    ...
    print("修改后的轨迹：",total_traj[:attack_length])
    #insert data
