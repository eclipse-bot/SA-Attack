...
def hard_constraint(observe_trace_array, perturbation_tensor, hard_bound, physical_bounds):
    #hard_bound表示扰动位置变化多少
    if not isinstance(perturbation_tensor, np.ndarray):#判断类型是否为np.ndarray，并转换类型
    ...
    while not check_pass:
        ...
        merged_trace_array[:perturbation_array.shape[0],:] += theta * perturbation_array#获得前面4个轨迹
        ...
        check_pass = (np.sum(deviation > 1.5) == 0)
